...
struct Context
{
    ...
    Context(Redland::World &world, Redland::Namespaces &namespaces, Redland::Model &model, const std::string &path, const void *user_data = 0) : world(world), namespaces(namespaces), model(model), path(path), user_data(user_data) { }
    Context(const Context &ctx) : world(ctx.world), namespaces(ctx.namespaces), model(ctx.model), path(ctx.path), user_data(ctx.user_data) { }
    Context(const Context &ctx, const std::string &path) : world(ctx.world), namespaces(ctx.namespaces), model(ctx.model), path(path), user_data(ctx.user_data) { }
};
